...
`CLAUDE.md`: one HTML file, zero runtime dependencies, the data block only
ever regenerated by `refresh_data.py`, every stat fed the time-weighted
series, engine changes proven in Node before shipping.
...
- [ ] **Universe expansion** — factor ETFs (VTV/VUG/MTUM/QUAL), TIPS (TIP),
      ...
      `index.html`; keep the curated feel rather than becoming a ticker
...
- [ ] **Automated monthly refresh** — a scheduled GitHub Action that runs
      ...
      the regenerated `index.html`. Removes the manual refresh chore; the
